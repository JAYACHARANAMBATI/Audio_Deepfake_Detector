...
DEMO_USERS = {
    ...
    "test": {"password": "test123", "name": "Test User", "role": "User"},
}
...
def authenticate(username: str, password: str) -> bool:
    ...
    if username in DEMO_USERS and DEMO_USERS[username]["password"] == password:
        ...
        st.session_state.user_info = DEMO_USERS[username]
        return True
    return False
...
def show_login_page():
    ...
    with col2:
        ...
        <div class="login-header-box">
            ...
            <p>AI-Powered Voice Authenticity Verification</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with st.form("login_form", clear_on_submit=False):
            ...
            with col_btn2:
                ...
                demo_login = st.form_submit_button("⚡ Quick Demo", use_container_width=True)
            ...
            if demo_login:
                ...
                if authenticate("demo", "demo123"):
                    st.success("✅ Logged in with demo account!")
                    st.rerun()
        
        st.markdown('</div>', unsafe_allow_html=True)
        
        # Show demo credentials with better styling
        st.markdown("""
        <div class="demo-credentials-box">
            <h4>📋 Demo Credentials</h4>
            <div class="credential-item">👤 admin / admin123 &nbsp;→&nbsp; Administrator</div>
            <div class="credential-item">👤 demo / demo123 &nbsp;→&nbsp; User</div>
            <div class="credential-item">👤 test / test123 &nbsp;→&nbsp; User</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="footer-text">
            <p>🔒 This is a demo application for testing purposes</p>
            <p>Powered by <strong>Google Gemini AI</strong> | Audio Forensics Analysis</p>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    with header_col2:
        st.markdown(f"""
        <div style="text-align: right; padding: 1rem; background: #f0f2f6; border-radius: 10px; margin-top: 1rem;">
            <p style="margin: 0; font-size: 0.9rem;">👤 <strong>{st.session_state.user_info['name']}</strong></p>
            <p style="margin: 0; font-size: 0.75rem; color: #666;">{st.session_state.user_info['role']}</p>
        </div>
        """, unsafe_allow_html=True)
